...
mqttClient.on("message", (topic, message) => {
  ...
  if (topic === "lintas_alam/dataset_names") {
    ...
    const existingIndex = attendanceData.findIndex(
      (item) => item.name === msg.name && item.course === data.course
    );
    if (existingIndex !== -1) {
...
function updateAttendanceTable() {
  ...
  let html =
    "<table><tr><th>No</th><th>Nama</th><th>Waktu</th><th>Status</th><th>Mata Kuliah</th></tr>";
...
function updateTotalAttendance() {
  ...
  ).textContent = `${total} Mahasiswa`;
}

function updateLineChart() {
  const ctx = document.getElementById("attendanceLineChart").getContext("2d");
  const timestamps = [
    ...new Set(attendanceData.map((data) => data.timestamp)),
  ].sort();
  const attendanceCount = timestamps.map(
    (t) => attendanceData.filter((data) => data.timestamp === t).length
  );
...
function updateBarChart() {
  ...
  courses.forEach((course) => {
    if (course !== "Tidak ada jadwal") {
      courseAttendance[course] = attendanceData.filter(
...
function updateRecentLogs() {
  ...
  const recent = attendanceData
    .slice(0, 5)
    .map((data) => `${data.name} - ${data.timestamp} (${data.course})`)
    .reverse();
  recent.forEach((log) => {
    const p = document.createElement("p");
    p.textContent = log;
...
function deleteIndividualSchedule() {
  ...
  if (person) {
    publishCommand(
      "lintas_alam/schedule",
      JSON.stringify({ person: "delete", name: person })
    );
